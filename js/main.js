...
function dataLine(na, start, end, max, min, height, width, x0, y0, linewidth, color, opacity, animationTime, drawScale)
{
    ...
    console.log("window: "+window+"\tstart: "+start+"\tend: "+end+"\tsize: "+(end-start)+"\tgraphWidth: "+width);
    //console.log("x0: "+x0);
    //INITIAL DATA LINE
    var p="M 10 50";
    for(i=0;i<100;i++)
...
function ticks(fullLength, x0, y0, height, width, startTick, numTicks, color, opacity, mean, max, min)
    ...
    console.log("fullLength: "+fullLength+"\tnumZeroes: "+numZeroes+"\tnumTicks: "+numTicks);
    ...
    this.handleEvent=function(evt)
        ...
        switch(event.type)
            ...
            case 'change':
                ...
                //Remove previous visual elements if any
                ...
                console.log("Time spent sending"+ (new Date()-startTime));
                ...
                var p="";
                console.log("Time spent preprocessing"+ (new Date()-startTime));
